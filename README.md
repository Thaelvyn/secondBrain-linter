...
sblint [flags] <vault-path>
  ...
  --fix       apply best-effort deterministic fixes (R19 children_counts, R3 missing folder notes, R11 related wikilinks, R25 relation mirrors), then re-lint and report residuals
  ...
  --version   print version (ldflags-injected, default v0.3.0)
  -h / --help
```
...
## Rules (ADR 0004, rules 1–25; 25 numbered, 24 active — R14 retired)
...
| 10 | error | All wikilinks `[[...]]` resolve to existing files (alias/anchor stripped; target, target+".md") |
...
| 24 | error | Person folder notes' body `## Relations` section holds well-formed typed edges (`- <kind> [[<target>]]`, kind in taxonomy `relation_kinds`, target resolves to another person folder note), no duplicates/self-loops; prose after the link is ignored; `met`/`self` boolean and person-only; at most one `self` owner sharing a `name` and fully cross-linked in `related` (report-only, no fix) |
| 25 | warning | Every people relation edge is mirrored on its target (symmetric kind, else `inverse(kind)`); missing mirror appended to the target's body `## Relations` section by `--fix` |
...
- **R10 resolution order**: target → target+".md". Wikilinks resolve to files only; a directory path is not a resolution (matches Obsidian — no folder-notes plugin is installed, folder references are an open question). Embeds `![[` are skipped by R10 and reported by R12 only.
...
- **R23**: every `entities` frontmatter path that has a folder note must be inline-linked from the entry body, where the link target is the folder-note FILE path (`[[{entity}/{basename}]]`, e.g. `[[work/emi/people/rasa/rasa|Rasa]]`). A folder note exists when `<entities-path>` is a dir containing `<basename>.md` (Obsidian folder-note convention) or when the plain file `<entities-path>.md` exists; entities without a note are ignored (stubs are created on demand, ADR 0001). The body is searched for inline wikilinks only (embeds `![[` skipped, as in R10); alias `|…` and anchor `#…` are stripped and the note path, note path+".md" and its resolved path all match, so `[[path|Name]]`, `[[path#anchor]]` and `[[path.md]]` count. A bare folder path (`[[work/emi/people/rasa]]`) does NOT count — consistent with Obsidian's file-only resolution. The frontmatter `related` field does not count — but a `## Related` body list item does (a body link anywhere satisfies R23). One finding per unlinked entity: **error** when the folder note is `type: entity` (or its kind is missing/unreadable), **warning** when it is `type: collection`. Rationale: the julien vault's entity notes are overwhelmingly `type: entity` (people, projects), where a missing inline link is a real content defect; collection paths (`personal/health`, `personal/home`, `personal/decisions`) have folder notes too, and flagging them as hard errors would be dishonest — they are legitimate broad-context references. Daily-log files (including `daily-logs/in/`) and `_system/` files are exempt.
- **R24/R25 scoping**: both rules look only at folder notes whose dir is an entity dir directly under a `people` category of kind `entity` in the taxonomy (`personal/people/{entity}`, `work/{company|_shared}/people/{entity}`). A target is a *person folder note* when it is a folder-note file whose frontmatter `type: entity`. The typed relations live in an optional body `## Relations` section (not frontmatter: Obsidian does not graph-index wikilinks embedded in mixed-text properties, only whole-value `[[...]]` properties — forum 72602 — while the normal body link scanner indexes them). Only the item prefix is machine-parsed: `- <kind> [[<target>]]` with a lowercase alias-free kind from the taxonomy `relation_kinds` catalog and a target resolving via the file-only resolver (target, target+".md") to another person folder note — not the note itself, no `(kind, target)` duplicates. Any prose after the closing `]]` is free human text (non-temporal qualifiers) and is ignored. **R24** (error, report-only): every section item must satisfy the prefix grammar; malformed items (no kind, missing link, non-wikilink), unknown kinds, broken targets, self-loops and duplicates are errors. The old frontmatter `relations:` field no longer exists in the schema; a stale one is ignored. `met`/`self` must be booleans and may only appear on person folder notes. All `self: true` notes across the vault must carry the same `name` (≤1 owner; differing names are an error) and each must list every other `self` note in `related`. **R25** (warning, fixable): every edge A —K→ B requires B to carry the mirror edge — K for symmetric kinds, else `inverse(K)` — pointing back at A by canonical resolved path. Missing mirrors are completeness drift (same class as R19); `--fix` appends the missing item to B's `## Relations` section (creating the section at the end of the file when absent), preserving existing items byte-for-byte. Both rules no-op when the taxonomy has no `relation_kinds` (vaults without the key stay valid). Unknown kinds, broken targets and malformed items are R24 errors and never appear as R25 warnings.
- **Fix scope** (`--fix`): (a) R19 — every linted folder note's `children_counts` is rewritten to the actual map (added if missing; the rest of the frontmatter is kept byte-identical); (b) R3 — a missing folder note `{dir}/{dir}.md` is created (type entity/collection per taxonomy, name = title-cased dir, scope, `status: active`, `created: today`, actual `children_counts`); (c) R11 — bare-path `related` items are wrapped in `[[…]]`; (d) R25 — missing relation mirrors are appended as `- <kind> [[<source>]]` to the target note's body `## Relations` section (created at the end of the file when absent). Slug renames stay report-only: renaming files is not a content-safe deterministic operation (links, entities and source paths would silently break), so R1 stayed unfixed. All fixes are idempotent — a second `--fix` changes nothing — and never overwrite content outside the fields/sections above. After fixing, sblint re-scans, re-runs the rules and reports residual findings.
- **`--recount`** is the focused R19 fix: it rewrites `children_counts` only, prints which notes were updated ("recounted N file(s)"), and is idempotent. `--fix --recount` behaves exactly like `--recount` (a subset of `--fix`); R25 mirror appends never run under `--recount`.
...
- Rules 19–22 and the three fix modes were **shipped in v0.2.0** (ADR 0006 adds rule 22; the ADR 0004 consistency group is now fully implemented). v0.2.1 extends rule 11 to also validate the optional body `## Related` section (wikilinks only). v0.2.2 retires rule 14 (empty-entity-stub warning) per ADR 0004 addendum 5; rule numbering unchanged (R15–R22 still emit). v0.2.3 adds rule 23 (entry body must inline-link each entity with a folder note; ADR 0004), report-only (no `--fix`). v0.2.4 aligns link resolution with Obsidian: wikilinks resolve to **files only** (target, target+".md") — a folder path no longer resolves to its folder note, so R10 flags `[[work/emi/people/rasa]]` and R23 requires the folder-note file path `[[work/emi/people/rasa/rasa]]` (or `.md`). v0.3.0 adds typed people relationships (ADR 0004 addendum 8): taxonomy `relation_kinds`, rule 24 (body `## Relations` well-formedness, error, report-only) and rule 25 (relation mirrors, warning, `--fix`-repairable).
